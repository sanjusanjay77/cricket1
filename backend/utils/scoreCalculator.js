...
The information is reconstructed from the
complete balls table.
*/
...
function buildFallOfWickets(
  ...
  for (const ball of balls) {
    ...
    if (
      ...
      const legalBallsBeforeThisBall =
        balls
          .filter(
            b =>
              Number(
...
Every wicket finishes the current partnership.

The next partnership starts from the first ball
recorded after that wicket.

This means the result survives refresh and undo
because it is reconstructed from the balls table.
*/
...
function buildPartnerships(
  ...
  function finishPartnership(
    isCurrent = false
  ) {
    /*
    Do not create empty partnerships.
    */
    if (
      partnershipRuns === 0 &&
      partnershipBalls === 0 &&
  ...
  for (const ball of balls) {
    ...
    The wicket ball's runs belong to the
    partnership that just ended.
    */
    if (
      ball.is_wicket &&
      ball.dismissed_id
    ) {
      finishPartnership(false);
  ...
  /*
  The remaining partnership is the current
  partnership.
  */
  if (
    partnershipRuns > 0 ||
    ...
    finishPartnership(true);
  }

  /*
  If the last partnership was completed by a wicket,
  mark all previous partnerships as completed.
  */
  if (
    ...
    /*
    If the innings ended with a wicket,
    there should not be an active partnership.
    */
    const lastBall =
  ...
  */
  ...
  return {
    ...
    */
    ...
    /*
    NEW:
    Complete partnership history.
    */
    partnerships,

    /*
    NEW:
    Complete fall-of-wickets history.
    */
    fallOfWickets,

    /*
    NEW:
    Explicit current partnership.
    */
    currentPartnership,
...
async function finalizeMatch(matchId) {
  ...
  const score1 = Number(inn1.total_runs || 0);
  const score2 = Number(inn2.total_runs || 0);
  const wickets2 = Number(inn2.total_wickets || 0);

  if (score2 > score1) {
    const wicketsInHand =
      Math.max(0, MAX_WICKETS - wickets2);
...
async function getScoreboard(inningsId) {
  ...
  for (const ball of balls) {
    ...
    if (bowl) {
      ...
      if (!overRuns[ball.bowler_id][ball.over_number]) {
...
async function computeCareerBattingStats(playerId) {
  ...
  const timesOut = Number(outRow?.c || 0);
  const inningsBatted = Number(inningsRow?.c || 0);
...
async function getAllTimeRecords() {
  ...
  for (const innings of inningsRows) {
    ...
    for (const bowling of scoreboard.bowlingCard || []) {
      ...
      const runs =
        Number(bowling.runs || 0);

      if (
        !bestBowling ||
        wickets >
          Number(bestBowling.wickets || 0) ||
        (
          wickets ===
            Number(bestBowling.wickets || 0) &&
          runs <
            Number(bestBowling.runs || 0)
        )
      ) {
        bestBowling = {
          player_id: playerId,
  ...
  const battingLeaders =
    Object.values(battingMap);

  const bowlingLeaders =
    Object.values(bowlingMap);
