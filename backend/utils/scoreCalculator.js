 ...
 */
function recordBall(inningsId, payload) {
  const innings = getInnings(inningsId);
  ...
  const ballCountRow = db.prepare('SELECT COUNT(*) AS c FROM balls WHERE innings_id = ?').get(inningsId);
  ...
  checkAndFinalizeInnings(inningsId);
...
function undoLastBall(inningsId) {
  const last = db.prepare('SELECT * FROM balls WHERE innings_id = ? ORDER BY ball_sequence DESC LIMIT 1').get(inningsId);
  if (!last) throw new Error('No balls to undo');
  db.prepare('DELETE FROM balls WHERE id = ?').run(last.id);
  recomputeInningsFromBalls(inningsId);
  // Un-complete the innings if it had been marked complete
  db.prepare('UPDATE innings SET is_completed = 0 WHERE id = ?').run(inningsId);
...
function recomputeInningsFromBalls(inningsId) {
  const innings = getInnings(inningsId);
  const balls = db.prepare('SELECT * FROM balls WHERE innings_id = ? ORDER BY ball_sequence ASC').all(inningsId);
...
function checkAndFinalizeInnings(inningsId) {
  const innings = getInnings(inningsId);
  const match = getMatch(innings.match_id);
  ...
  if (allOut || oversDone || targetReached) {
    db.prepare('UPDATE innings SET is_completed = 1 WHERE id = ?').run(inningsId);
    ...
    if (innings.innings_number >= 2 || match.overs_limit === 0) {
      ...
      db.prepare('UPDATE matches SET status = ? WHERE id = ?').run('innings-break', match.id);
    }
  }
}
...
function finalizeMatch(matchId) {
  ...
  const allInnings = db.prepare('SELECT * FROM innings WHERE match_id = ? ORDER BY innings_number ASC').all(matchId);
  const inn1 = allInnings.find(i => i.innings_number === 1);
  const inn2 = allInnings.find(i => i.innings_number === 2);
  if (!inn1 || !inn2) return;

  const team1 = db.prepare('SELECT * FROM teams WHERE id = ?').get(inn1.batting_team_id);
  const team2 = db.prepare('SELECT * FROM teams WHERE id = ?').get(inn2.batting_team_id);
  ...
  db.prepare('UPDATE matches SET status = ?, result_text = ?, winner_id = ? WHERE id = ?')
    .run('completed', resultText, winnerId, matchId);
}

/** Full batting scorecard for an innings, derived entirely from the balls table. */
function computeBattingScorecard(inningsId) {
  const balls = db.prepare('SELECT * FROM balls WHERE innings_id = ? ORDER BY ball_sequence ASC').all(inningsId);
...
function computeBowlingScorecard(inningsId) {
  const balls = db.prepare('SELECT * FROM balls WHERE innings_id = ? ORDER BY ball_sequence ASC').all(inningsId);
  const stats = {};
  const order = [];
...
function computeCurrentPartnership(inningsId) {
  const balls = db.prepare('SELECT * FROM balls WHERE innings_id = ? ORDER BY ball_sequence ASC').all(inningsId);
...
function getScoreboard(inningsId) {
  const innings = getInnings(inningsId);
  if (!innings) return null;
  const battingCard = computeBattingScorecard(inningsId);
  const bowlingCard = computeBowlingScorecard(inningsId);
  const recentBalls = db.prepare('SELECT * FROM balls WHERE innings_id = ? ORDER BY ball_sequence DESC LIMIT 12').all(inningsId).reverse();
  return {
    innings,
    ...
    bowlingCard,
    recentBalls,
    partnership: computeCurrentPartnership(inningsId),
 ...
 */
function computeCareerBattingStats(playerId) {
  const balls = db.prepare('SELECT * FROM balls WHERE batsman_id = ?').all(playerId);
  ...
  const timesOut = db.prepare('SELECT COUNT(*) AS c FROM balls WHERE dismissed_id = ? AND is_wicket = 1').get(playerId).c;
  const inningsBatted = db.prepare(
 ...
 * ever recorded.
 */
function computeCareerBowlingStats(playerId) {
  const balls = db.prepare('SELECT * FROM balls WHERE bowler_id = ?').all(playerId);
  ...
  const inningsBowled = db.prepare('SELECT COUNT(DISTINCT innings_id) AS c FROM balls WHERE bowler_id = ?').get(playerId).c;
...
function getPlayerCareerStats(playerId) {
  return {
    batting: computeCareerBattingStats(playerId),
    bowling: computeCareerBowlingStats(playerId),
  };
}
...
function getAllTimeRecords() {
  const inningsRows = db.prepare('SELECT id, match_id FROM innings').all();

  let highestScore = null;
  let bestBowling = null;

  for (const inn of inningsRows) {
    for (const b of computeBattingScorecard(inn.id)) {
    ...
    for (const b of computeBowlingScorecard(inn.id)) {
  ...
  const batsmanIds = db.prepare('SELECT DISTINCT batsman_id AS id FROM balls').all().map(r => r.id);
  const bowlerIds = db.prepare('SELECT DISTINCT bowler_id AS id FROM balls').all().map(r => r.id);

  const battingLeaders = batsmanIds.map(id => ({ player_id: id, ...computeCareerBattingStats(id) }));
  const bowlingLeaders = bowlerIds.map(id => ({ player_id: id, ...computeCareerBowlingStats(id) }));
