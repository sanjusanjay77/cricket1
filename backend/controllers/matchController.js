...
exports.createMatch = (req, res) => {
  ...
  const id = uuidv4();
  db.prepare(`INSERT INTO matches (id, team1_id, team2_id, match_type, overs_limit, venue, match_date, status)
              VALUES (?, ?, ?, ?, ?, ?, ?, 'upcoming')`)
...
exports.setToss = (req, res) => {
  ...
  db.prepare('UPDATE matches SET toss_winner_id=?, toss_decision=?, status=? WHERE id=?')
    .run(toss_winner_id, toss_decision, 'live', req.params.id);
  ...
  db.prepare(`INSERT INTO innings (id, match_id, innings_number, batting_team_id, bowling_team_id)
              VALUES (?, ?, 1, ?, ?)`).run(inningsId, match.id, battingFirstId, bowlingFirstId);

  res.json({ match: db.prepare('SELECT * FROM matches WHERE id = ?').get(match.id), innings_id: inningsId });
};

/** Start the 2nd innings once the 1st has finished (innings-break status). */
exports.startSecondInnings = (req, res) => {
  const match = db.prepare('SELECT * FROM matches WHERE id = ?').get(req.params.id);
  if (!match) return res.status(404).json({ error: 'Match not found' });
  const inn1 = db.prepare('SELECT * FROM innings WHERE match_id = ? AND innings_number = 1').get(match.id);
  if (!inn1 || !inn1.is_completed) return res.status(400).json({ error: 'First innings has not finished yet' });

  const inningsId = uuidv4();
  db.prepare(`INSERT INTO innings (id, match_id, innings_number, batting_team_id, bowling_team_id, target)
              VALUES (?, ?, 2, ?, ?, ?)`)
    .run(inningsId, match.id, inn1.bowling_team_id, inn1.batting_team_id, inn1.total_runs + 1);
  db.prepare('UPDATE matches SET status = ?, current_innings = 2 WHERE id = ?').run('live', match.id);
...
exports.getMatchDetail = (req, res) => {
  ...
  const inningsRows = db.prepare('SELECT * FROM innings WHERE match_id = ? ORDER BY innings_number ASC').all(match.id);
  const innings = inningsRows.map(i => getScoreboard(i.id));
